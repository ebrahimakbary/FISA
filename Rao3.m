...
for run_no=1:10
    ...
    for it=1:MaxIt
        ...
        for i=1:nPop
            ...
            Step =abs(pop(i).Position) - (pop(j).Position);
            if pop(j).Cost < pop(i).Cost
                Step =abs(pop(j).Position) - (pop(i).Position);
            end
            
            newsol.Position =pop(i).Position+ rand(VarSize).*(BBest.Position - abs(Wworst.Position)) + rand(VarSize).*Step;
